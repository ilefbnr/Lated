# =============================================================================
# lated.detection.protocols.rdp_detector — RDP lateral movement heuristic
# =============================================================================
#
# Mirrors `smb_detector.py`. Targets MITRE T1021.001 (Remote Desktop Protocol).
# Emits a ReconScore when a host opens RDP sessions to multiple distinct
# targets inside a short window — the classic Windows admin-pivot signature.
#
# Why a dedicated detector vs. the generic ReconDetector:
#   - RDP has a single fixed port (3389) — the recon heuristic loses signal
#     in the noise of many other port-diverse probes.
#   - Severity escalation on critical hosts (DC, jump host) needs a
#     protocol-tagged signal so SeverityMapper / explainability can tell
#     "RDP to DC" apart from "any random fanout".
# =============================================================================
...
class RDPDetector:
    """Heuristic RDP detector over CanonicalFlow windows."""

    def __init__(
        self,
        window_seconds: int = 60,
        thresholds: RDPThresholds | None = None,
    ):
        self.window = SlidingWindow(size_seconds=window_seconds)
        self.thresholds = thresholds or RDPThresholds()
    ...
    def _emit_completed_window(self) -> Iterator[ReconScore]:
        ...
        for subject in self.window.subjects():
            flows = [flow for flow in self.window.flows_for(subject) if flow.dst_port in RDP_PORTS]
            if not flows:
                continue
            ...
            signals = ["rdp_remote_access"]
            if len(unique_targets) >= self.thresholds.min_unique_targets:
                signals.append("rdp_fanout")
            if cross_subnet:
                signals.append("cross_subnet_rdp")

            yield ReconScore(
                detector=DetectorName.RDP,
